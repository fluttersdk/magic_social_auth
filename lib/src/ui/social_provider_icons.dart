import 'package:flutter/foundation.dart' show immutable;

/// Default UI metadata for a social auth provider.
@immutable
class SocialProviderDefaults {
  final String label;
  final String iconSvg;
  final String? iconClassName;
  final int order;

  const SocialProviderDefaults({
    required this.label,
    required this.iconSvg,
    this.iconClassName,
    this.order = 99,
  });
}

/// Built-in SVG icons and UI defaults for social auth providers.
///
/// Provides default icons for Google, Microsoft, and GitHub.
/// Custom providers can register their own defaults via [register].
class SocialProviderIcons {
  SocialProviderIcons._();

  // -- Built-in SVG constants --
  static const String googleSvg =
      '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 0 1-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" fill="#4285F4"/><path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853"/><path d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" fill="#FBBC05"/><path d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" fill="#EA4335"/></svg>';

  static const String microsoftSvg =
      '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><rect x="1" y="1" width="10" height="10" fill="#F25022"/><rect x="13" y="1" width="10" height="10" fill="#7FBA00"/><rect x="1" y="13" width="10" height="10" fill="#00A4EF"/><rect x="13" y="13" width="10" height="10" fill="#FFB900"/></svg>';

  static const String githubSvg =
      '<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" clip-rule="evenodd" d="M12 2C6.477 2 2 6.477 2 12c0 4.42 2.865 8.17 6.839 9.49.5.092.682-.217.682-.482 0-.237-.009-.866-.013-1.7-2.782.604-3.369-1.34-3.369-1.34-.454-1.156-1.11-1.464-1.11-1.464-.908-.62.069-.607.069-.607 1.003.07 1.531 1.03 1.531 1.03.892 1.529 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.11-4.555-4.943 0-1.091.39-1.984 1.029-2.683-.103-.253-.446-1.27.098-2.647 0 0 .84-.269 2.75 1.025A9.578 9.578 0 0 1 12 6.836a9.59 9.59 0 0 1 2.504.337c1.909-1.294 2.747-1.025 2.747-1.025.546 1.377.203 2.394.1 2.647.64.699 1.028 1.592 1.028 2.683 0 3.842-2.339 4.687-4.566 4.935.359.309.678.919.678 1.852 0 1.336-.012 2.415-.012 2.743 0 .267.18.578.688.48C19.138 20.167 22 16.418 22 12c0-5.523-4.477-10-10-10z" fill="currentColor"/></svg>';

  /// Built-in defaults for known providers.
  static const Map<String, SocialProviderDefaults> _builtIn = {
    'google': SocialProviderDefaults(
      label: 'Google',
      iconSvg: googleSvg,
      order: 1,
    ),
    'microsoft': SocialProviderDefaults(
      label: 'Microsoft',
      iconSvg: microsoftSvg,
      order: 2,
    ),
    'github': SocialProviderDefaults(
      label: 'GitHub',
      iconSvg: githubSvg,
      iconClassName: 'fill-slate-900 dark:fill-white',
      order: 3,
    ),
  };

  /// Custom provider defaults registered at runtime.
  static final Map<String, SocialProviderDefaults> _custom = {};

  /// Register custom provider UI defaults.
  static void register(String provider, SocialProviderDefaults defaults) {
    _custom[provider] = defaults;
  }

  /// Get defaults for a provider.
  /// Priority: custom registry > built-in defaults > null.
  static SocialProviderDefaults? forProvider(String provider) {
    return _custom[provider] ?? _builtIn[provider];
  }

  /// Check if defaults exist for a provider.
  static bool has(String provider) {
    return _custom.containsKey(provider) || _builtIn.containsKey(provider);
  }

  /// Reset custom registrations (for testing).
  static void reset() {
    _custom.clear();
  }
}
