...
You are preparing a new release for the **Magic Social Auth** Flutter plugin. Follow this checklist precisely:
...
3. **Categorize** — Use these emoji categories:
   - `### ✨ New Features` — new drivers, UI components, handler implementations
   - `### 🐛 Bug Fixes` — bug fixes
...
3. **Triggers `publish.yml`** — tag push starts the `publish` job (publish to pub.dev via OIDC)
...
**Full Changelog**: https://github.com/fluttersdk/magic_social_auth/compare/{previous_tag}...{version}
NOTES
)"
```
...
**GitHub Release:** https://github.com/fluttersdk/magic_social_auth/releases/tag/{version}
...
**pub.dev:** ✅ Validate passed → Published (run #{publish_run_id}) — https://pub.dev/packages/magic_social_auth
```
